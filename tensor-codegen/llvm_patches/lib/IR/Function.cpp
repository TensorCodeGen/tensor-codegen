...
std::string Intrinsic::getName(ID id, ArrayRef<Type*> Tys) {
  ...
  errs() << "GET NAME: " << Result << "\n";
  for (Type *Ty : Tys) {
    auto MangledStr = getMangledTypeStr(Ty);
    if (MangledStr != "")
    	Result += "." + MangledStr;//getMangledTypeStr(Ty);
  }
 errs() << "RESULT: " << Result << "\n";
  return Result;
}
...
Function *Intrinsic::getDeclaration(Module *M, ID id, ArrayRef<Type*> Tys) {
  ...
  errs() << getType(M->getContext(), id, Tys) << "\n";
  errs() << "FUNCTION: NAME: " << getName(id, Tys) << "\n";
